...
//YHT 1 changed force distribution

    Point<dim> forcecenter;
    forcecenter(0) = 0;
    ...
    for (unsigned int point_n = 0; point_n < points.size(); ++point_n)
      {
        ...
        //if (((points[point_n] - forcecenter).norm_square() > 2300 * 2300) ||
        //    ((points[point_n] - forcecenter).norm_square() < 2400 * 2400))
        //  values[point_n][0] = 1000;
        //else
        //  values[point_n][0] = 0;
    	const double cGM = 2.5786 * 0.001 * 0.001 * 4.7 * 0.001;

//F1


    	values[point_n][0] = (-6) * cGM * points[point_n][0];
    	values[point_n][1] = 3 * cGM * points[point_n][1];
    	values[point_n][2] = 3 * cGM * points[point_n][2];


//F2


//    	values[point_n][0] = (-6) * cGM * points[point_n][1];
//    	values[point_n][1] = (-6) * cGM * points[point_n][0];
//    	values[point_n][2] = 0;
...
//YHT 1 changed force distribution
  ...
  template <int dim>
    void lambda_value_list(const std::vector<Point<dim>> &points,
      ...
      for (unsigned int point_n = 0; point_n < points.size(); ++point_n)
        {
          ...
          // 0.2 around one of these points, then set the force in x-direction
          // to one, otherwise to zero:
          if ((sqrt((points[point_n] - forcecenter).norm_square()) > 220000) ||
              (sqrt((points[point_n] - forcecenter).norm_square()) < 250000))
            values[point_n] = 576900000;
          else
            values[point_n] = 576900;
...
//YHT 1.1 changed lambda
  ...
  template <int dim>
    void mu_value_list(const std::vector<Point<dim>> &points,
                         std::vector<double> &  values)
    {
      AssertDimension(values.size(), points.size());
      Assert(dim >= 2, ExcNotImplemented());
      ...
      for (unsigned int point_n = 0; point_n < points.size(); ++point_n)
        {
          ...
          if ((sqrt((points[point_n] - forcecenter).norm_square()) > 220000) ||
              (sqrt((points[point_n] - forcecenter).norm_square()) < 250000))
            values[point_n] = 384600000;
          else
            values[point_n] = 384600;

//YHT 1.2 changed lambda
  ...
  template <int dim>
       void lambda_face_value_list(const std::vector<Point<dim>> &points,
                            std::vector<double> &  values)
       {
         AssertDimension(values.size(), points.size());
         Assert(dim >= 2, ExcNotImplemented());
   ...
   //YHT 1.1 changed lambda
     ...
     template <int dim>
       void mu_face_value_list(const std::vector<Point<dim>> &points,
                            std::vector<double> &  values)
       {
         AssertDimension(values.size(), points.size());
         Assert(dim >= 2, ExcNotImplemented());
...
//YHT change BC close Diriclet BC
...
//YHT BC-add face variables
    ...
    std::vector<double> mu_face_values(n_face_q_points);
...
//YHT 2 chenged lambda and mu

    //Functions::ConstantFunction<dim> lambda(576900000.), mu(384600000.);

//YHT 2 changed lambda and mu
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        lambda_value_list(fe_values.get_quadrature_points(), lambda_values);
        mu_value_list(fe_values.get_quadrature_points(), mu_values);
...
//YHT 3 changed solver_control stop condition
...
//YHT 4 changed mesh to hypershell and changed refine loops

	Point<dim> center;
    center[0] = 0;
    const double inner_radius = 220000, outer_radius = 250000;
	  for (unsigned int cycle = 0; cycle < 2; ++cycle)
      {
        ...
        if (cycle == 0)
          {
            GridGenerator::hyper_shell(triangulation, center, inner_radius, outer_radius, 24);
